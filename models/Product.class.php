 ...
 * Date: 02.03.2022
 */
class Product extends mAbstracts\Model
{
    ...
    public static function getProducts(): ?array
    {
        ...
        $products = lib\DBContext :: getInstance() -> Select($sql);

        return $products && count($products) > 0 ? $products : null;
    }
    ...
    public static function getProduct(int $productId) : ?array
    {
        ...
        $product = lib\DBContext :: getInstance() -> Select($sql, ['productid' => $productId]);

        return $product && count($product) > 0 ? $product : null;
    }
     ...
     * @return int возвращает ID товара, который был добавлен.
     */
    ...
    public static function removeProduct(int $productId) : void
    {
        lib\DBContext :: getInstance() -> Query(
            'UPDATE products SET ismark = 1 WHERE productid = :productid',
            ['productid' => $productId]);
    }
}