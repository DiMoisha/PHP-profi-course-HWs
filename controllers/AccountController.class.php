    ...
    class AccountController extends Controller
    {
        ...
        function Login($data): ?string
        {
            ...
            if ($this -> isPost()) {
                ...
                if ($login && $passwd) {
                    $auth = Account :: checkAccount($login, $passwd);

                    if ($auth) {
                        Account :: setLoginTime($login);
        ...
        function Register($data): ?string
        {
            ...
            if ($this -> isPost()) {
                ...
                if ($login && $userName && $email) {
                    ...
                    $res = Account :: registerAccount($login, $passwd, $userName, $email);
            ...
            if ($_COOKIE['authuserid'] && $_COOKIE['authuser']) {
                if ($this -> isPost()) {
                    ...
                    if ($login && $passwd) {
                        ...
                        if ($auth) {
                            Account:: removeAccount($auth['userid']);
                            $this -> Logout();
                        } else {
                            return '<p class="text-danger">Вы ввели неверные данные! Попробуйте еще раз!</p><hr class="bhr" /><br />';
                        }
