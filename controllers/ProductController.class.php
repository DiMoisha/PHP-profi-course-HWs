 ...
 * Date: 02.03.2022
 */
class ProductController extends cBase\Controller
{
    public $pageTitle;
    ...
    function Index($data): ?array
    {
        ...
        return mProduct\Product:: getProducts();
    }
    ...
    function Product($data): array
    {
        ...
        $productId = isset($data['id']) ? intval(filter_var(trim($data['id']), FILTER_SANITIZE_STRING)) : 0;

        if ($productId > 0) {
            $product = mProduct\Product:: getProduct($productId);
    ...
    function Create($data)
    {
        ...
        if ($_COOKIE['authuserid'] && $_COOKIE['authuser'] && $_COOKIE['authroleid'] && $_COOKIE['authroleid'] == '1') {
            if ($this->isPost()) {
                ...
                $productId = mProduct\Product:: createProduct($product);
    ...
    function Edit($data)
    {
        if ($_COOKIE['authuserid'] && $_COOKIE['authuser'] && $_COOKIE['authroleid'] && $_COOKIE['authroleid'] == '1') {
            ...
            if ($this->isPost()) {
                ...
                mProduct\Product:: editProduct($product);
                mProduct\ProductPics::removePics($productId);
                ...
                if (count($extPics) > 0) {
                    foreach ($extPics as $picName) {
                        mProduct\ProductPics:: addPic($productId, $picName);
                    }
                }
    ...
    function Remove($data)
    {
        if ($_COOKIE['authuserid'] && $_COOKIE['authuser'] && $_COOKIE['authroleid'] && $_COOKIE['authroleid'] == '1') {
            ...
            if ($productId > 0) {
                mProduct\Product:: removeProduct($productId);
            } else {
                header('Location: /notfound');
            }
    ...
    private function loadProductPics(int $productId, array $pics)
    {
        foreach ($pics as $pic) {
            if ($pic && isset($pic['name']) && !empty($pic['name'])) {
                $picName = $pic['name'];
                $fileSize = $pic['size'];
                $img = lib\Config:: get('path_public') . "/images/products/" . $picName;
                $thumb = lib\Config:: get('path_public') . "/images/products/thumbnails/" . $picName;
